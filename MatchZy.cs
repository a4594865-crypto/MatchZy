...
namespace MatchZy
{
    ...
    public partial class MatchZy : BasePlugin
    {
        ...
        public bool isCountdownActive = false; // 全域開關：管理倒數期間的對話雜訊
        ...
        public int minimumReadyRequired = 2; // 開賽所需人數
        ...
        public override void Load(bool hotReload) {
                    ...
                    if (!isAllowed) {
                        AddTimer(1.5f, () => {
                            if (player != null && player.IsValid) {
                                Server.ExecuteCommand($"kickid {player.UserId} \"伺 服 器 白 名 單 已 開 啟，您 不 在 白 名單 中。\"");
            ...
            // 🎯 【黃金切換點攔截】：倒數結束、跳到刀局（熱身結束）的瞬間才進行洗牌與訊息發送！
            // 這完美解決了：1. round01.txt 與記分板完全同步 2. 訊息在你記憶中「倒數完才跳出」的完美體驗
            RegisterEventHandler<EventRoundPrestart>((@event, info) => {
                if (isShufflePending && !matchStarted && isWarmup) 
                {
                    // 在熱身正式關閉、刀局要初始化加載的這一瞬間執行洗牌！
                    ExecuteShuffleLogic(); 
                    UpdatePlayersMap();
                }
                return HookResult.Continue;
            ...
            RegisterEventHandler<EventPlayerChat>((@event, info) => {
                ...
                if (message == ".r" || message == ".ready") {
                    if (!matchStarted && readyAvailable && GetReadyPlayersCount() >= (minimumReadyRequired - 1)) {
                        ...
                        // 🎯 這裡完全不去洗牌，不加任何自訂計時器干擾！直接放行開賽
                        // 這樣一來，人數不足時原生的未準備廣播能百分之百正常運作。
        ...
        public void ExecuteShuffleLogic() 
        {
            ...
            // === 💥 【記憶體陣列預測】(完美解決 Demo、JSON 與 round01.txt 不同步問題) ===
            var predictedTCaptain = activePlayers.ElementAtOrDefault(0);
            var predictedCTCaptain = activePlayers.ElementAtOrDefault(half);

            string tName = (predictedTCaptain != null && !string.IsNullOrEmpty(predictedTCaptain.PlayerName)) ? $"team_{predictedTCaptain.PlayerName}" : "team_Terrorists";
            string ctName = (predictedCTCaptain != null && !string.IsNullOrEmpty(predictedCTCaptain.PlayerName)) ? $"team_{predictedCTCaptain.PlayerName}" : "team_CounterTerrorists";

            // 在熱身剛好關閉、刀局要載入的第 0 秒寫入隊名，這會被原生初始模組完美抓取
            Server.ExecuteCommand($"mp_teamname_1 \"{tName}\"");
            Server.ExecuteCommand($"mp_teamname_2 \"{ctName}\"");

            // 同步在聊天框刷出提示 (此時正值倒數結束、玩家在刀局新隊伍誕生的瞬間)
