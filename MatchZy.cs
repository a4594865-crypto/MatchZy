...
public void OnUnshuffleCommand(CCSPlayerController? player, CommandInfo command) {
    ...
    if (player == null) {
        ...
        // =========================================================================
        // 純粹隨機洗牌 + 非自殺換隊SwitchTeam（100% 採用您不卡秒的舊架構優化）
        // =========================================================================
          ...
          // 【核心修正】：維持原本完全沒有 NextFrame 的高流暢同步寫法，100% 延續您不卡倒數的特性！
            if (matchzyTeam1 != null && matchzyTeam2 != null)
            {
                ...
                if (string.IsNullOrWhiteSpace(matchzyTeam1.teamName) || matchzyTeam1.teamName == "team_")
                {
                    ...
                    {
                        // 2. 萬一他手懶剛進服名字是空的，立刻現場盲抓場上隨便一個名字已經載入好的真人
                        var fallbackPlayer = Utilities.GetPlayers().FirstOrDefault(p => p != null && p.IsValid && !p.IsBot && !string.IsNullOrWhiteSpace(p.PlayerName));
                        
                        // 3. 【徹底修正】：如果撈到別人就用別人的名字，真的撈不到（機率為0），就硬用原本隊長哪怕是空的名字，絕對不用死名字！
                        matchzyTeam1.teamName = fallbackPlayer != null ? $"team_{fallbackPlayer.PlayerName}" : $"team_{realCTPlayer?.PlayerName}";
                    }
                }

                // 修正 matchzyTeam2 (預設 T)
                if (string.IsNullOrWhiteSpace(matchzyTeam2.teamName) || matchzyTeam2.teamName == "team_")
                {
                    ...
                    else
                    {
                        ...
                        // 3. 【徹底修正】：如果撈到別人就用別人的名字，真的撈不到（機率為0），就硬用原本隊長哪怕是空的名字，絕對不用死名字！
                        matchzyTeam2.teamName = fallbackPlayer != null ? $"team_{fallbackPlayer.PlayerName}" : $"team_{realTPlayer?.PlayerName}";
                    }
                }

                // 【安全自定義同步】：用官方指令直接刷進計分板！
