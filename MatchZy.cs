...
if (matchStartCountdownTimer != null)
{
    ...
    CancelMatchCountdown($" 玩家{ChatColors.LightRed} {player.PlayerName} {ChatColors.White}斷開連線，倒數中止請重新輸入 {ChatColors.LightRed}.R {ChatColors.White}準備");
...
AddCommandListener("jointeam", (player, info) =>
{
    ...
    {
        // 顯示警告訊息給該玩家
        player.PrintToChat($"{chatPrefix} {ChatColors.LightRed}【警告】{ChatColors.Default}倒數期間禁止切換隊伍或觀戰！");
    ...
    if (matchStarted)
    {
        ...
        {
            player.PrintToChat($"{ChatColors.LightRed}【警告】{ChatColors.Default}比賽已開始，禁止互換隊伍");
            return HookResult.Stop; 
        }
    }
