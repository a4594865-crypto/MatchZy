...
namespace MatchZy
{
    ...
    public static class StringSimilarity
    {
        ...
        public static string FindNearestName(string inputName, List<string> names)
        {
            ...
            {
                // 【.NET 10 升級】：拔除 LINQ FirstOrDefault
                foreach (var name in names)
                {
                    if (name.StartsWith(inputName, StringComparison.OrdinalIgnoreCase))
                    {
                        return name;
                    }
                }
            ...
            string? nearestName = names.OrderByDescending(name => DiceCoefficient(inputName, name)).FirstOrDefault();
    ...
    public partial class MatchZy
    {
        ...
        public void GetSpawns()
        {
            ...
            foreach (var spawn in spawnsct)
            {
                ...
                if (spawn is { IsValid: true, Enabled: true } && spawn.Priority == minPriority)
                {
                    if (spawn.CBodyComponent?.SceneNode is { } node)
                    {
        ...
        private void HandleSpawnCommand(CCSPlayerController? player, string commandArg, byte teamNum, string command)
        {
            ...
            if (!string.IsNullOrWhiteSpace(commandArg))
            {
                if (int.TryParse(commandArg, out int spawnNumber) && spawnNumber >= 1)
                {
                    ...
                    if (player.PlayerPawn.Value is { } pawn)
                    {
                        pawn.Teleport(spawnsData[teamNum][spawnNumber].PlayerPosition, spawnsData[teamNum][spawnNumber].PlayerAngle, new Vector(0, 0, 0));
                    }
        ...
        {
            // 【.NET 10 升級】：Switch 表達式
            return nadeName switch
            {
        ...
        private void HandleSaveNadeCommand(CCSPlayerController? player, string saveNadeName)
        {
            ...
            if (!string.IsNullOrWhiteSpace(saveNadeName))
            {
                ...
                string playerSteamID = isSaveNadesAsGlobalEnabled == false ? player.SteamID.ToString() : "default";

                if (player.PlayerPawn.Value is not { CBodyComponent.SceneNode: not null } pawn) return;
                
                QAngle playerAngle = pawn.EyeAngles;
                Vector playerPos = pawn.CBodyComponent.SceneNode.AbsOrigin;
                string currentMapName = Server.MapName;
                
                if (pawn.WeaponServices?.ActiveWeapon.Value is not { } activeWeapon) return;
                string nadeType = GetNadeType(activeWeapon.DesignerName);
                        ...
                        if (savedNadesDict.ContainsKey(currentSteamID))
                        {
                            ...
                            if (savedNadesDict[currentSteamID].ContainsKey(nearestName))
                            {
                                ...
                                if (lineupInfo.ContainsKey("Map") && lineupInfo["Map"] == Server.MapName)
                                {
                                    ...
                                    if (player.PlayerPawn.Value is { } pawn)
                                    {
                                        pawn.Teleport(loadedPlayerPos, loadedPlayerAngle, new Vector(0, 0, 0));
                                    }
                                    ...
                                    // 【.NET 10 升級】：完美消滅第 674 行警告！使用 string? 來承接可能為 null 的字串
        ...
        {
            if (!isPractice || player is not { IsValid: true, PlayerPawn.Value: not null }) return;
        ...
        public void OnSpawnCommand(CCSPlayerController? player, CommandInfo command)
        {
            ...
            foreach (var list in spawnsData.Values) if (list.Count is 0) { emptySpawns = true; break; }
            if (emptySpawns) GetSpawns();
            
            if (player is not { PlayerPawn.IsValid: true }) return;
        ...
        private void AddBot(CCSPlayerController? player, bool crouch)
        {
            try
            {
                if (!isPractice || player is not { IsValid: true, PlayerPawn: { IsValid: true, Value: not null } }) return;
        ...
        private void SpawnBot(CCSPlayerController botOwner, bool crouch)
        {
            try 
            {
                ...
                foreach (var tempPlayer in playerEntities)
                {
                    ...
                    // 【.NET 10 升級】：模式匹配拆箱
                    if (tempPlayer.UserId is int userId)
                    {
                        if (!pracUsedBots.ContainsKey(userId) && unusedBotFound)
                        {
                            Log($"UNUSED BOT FOUND: {userId} EXECUTING: kickid {userId}");
                        ...
                        pracUsedBots[userId]["controller"] = tempPlayer;
                        pracUsedBots[userId]["position"] = botOwnerPosition;
                        pracUsedBots[userId]["owner"] = botOwner;
                        pracUsedBots[userId]["crouchstate"] = crouch;

                        if (crouch && tempPlayer.PlayerPawn.Value is { } pawn)
                        {
        ...
        public void TemporarilyDisableCollisions(CCSPlayerController p1, CCSPlayerController p2)
        {
            ...
            if (p1.PlayerPawn.Value is { } p1Pawn && p2.PlayerPawn.Value is { } p2Pawn)
            {
            ...
            {
                if (p1p is not { IsValid: true, Value.IsValid: true } || p2p is not { IsValid: true, Value.IsValid: true })
                {
        ...
        private static void ElevatePlayer(CCSPlayerController? player)
        {
            ...
            if (player.PlayerPawn.Value.CBodyComponent?.SceneNode is { } node)
            {
                player.PlayerPawn.Value.Teleport(new Vector(node.AbsOrigin.X, node.AbsOrigin.Y, node.AbsOrigin.Z + 80.0f), player.PlayerPawn.Value.EyeAngles, new Vector(0, 0, 0));
            }
        }
        ...
        public HookResult OnPlayerSpawn(EventPlayerSpawn @event, GameEventInfo info)
        {
            ...
            if (isPractice && player is { IsValid: true, IsBot: true, UserId: var userId })
            {
                if (pracUsedBots.ContainsKey(userId))
                {
                    if (pracUsedBots[userId]["position"] is Position botPosition)
                    {
                        player.PlayerPawn.Value?.Teleport(botPosition.PlayerPosition, botPosition.PlayerAngle, new Vector(0, 0, 0));
                        bool isCrouched = (bool)pracUsedBots[userId]["crouchstate"];
                        if (isCrouched && player.PlayerPawn.Value is { } pawnBot)
                        {
                            pawnBot.Flags |= (uint)PlayerFlags.FL_DUCKING;
                            CCSPlayer_MovementServices movementService = new(pawnBot.MovementServices!.Handle);
                            AddTimer(0.1f, () => movementService.DuckAmount = 1);
                            AddTimer(0.2f, () => pawnBot.Bot!.IsCrouching = true);
                        }
                        
                        if (pracUsedBots[userId]["owner"] is CCSPlayerController botOwner && botOwner is { IsValid: true, PlayerPawn: { IsValid: true } }) 
                        {
        ...
        public void OnFFCommand(CCSPlayerController? player, CommandInfo? command)
        {
            ...
            foreach (var key in playerData.Keys) {
                ...
                if (playerData[key].PlayerPawn.Value is { } pawn)
                {
        ...
        public void ResetFastForward(Dictionary<int, MoveType_t> preFastForwardMoveTypes) {
            ...
            foreach (var key in playerData.Keys) {
                if(!IsPlayerValid(playerData[key])) continue;
                if (playerData[key].PlayerPawn.Value is { } pawn && preFastForwardMoveTypes.ContainsKey(key))
                {
                    pawn.MoveType = preFastForwardMoveTypes[key];
                }
            }
        }
        ...
        public void OnNoFlashCommand(CCSPlayerController? player, CommandInfo? command) {
            if (!isPractice || player?.UserId is not int userId) return;
        ...
        {
            if (player.UserId is not int userId) return false;
        ...
        {
            if (!isPractice || player?.UserId is not int userId || player.PlayerPawn.Value is null) return;
...
public void OnLastIndexCommand(CCSPlayerController? player, CommandInfo? command)
{
    ...
    if (IsValidPositionForLastGrenade(player, 1))
    {
        ...
        public void TeleportPlayerToBestSpawn(CCSPlayerController player, byte teamNum)
        {
            ...
            if (player.PlayerPawn.Value?.CBodyComponent?.SceneNode is not { } node) return;
            
            Vector playerPosition = node.AbsOrigin;
            int closestIndex = -1;
